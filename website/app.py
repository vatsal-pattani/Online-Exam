...
def dashboard():
    ID = session['ID']
    courses = getCourses(ID)  # getCourses is unnecessary function. We can do it just by execute
...
def papers():
    ...
    papers=execute(f'''select E_id,duration,exam_time from Exam where (Exam.C_id = '{c_id[0][0]}'); ''')
...
def questions():
    ...
    return render_template("questions.html",questions=questions, tm=total_marks)   #Not passing correct ans and Q_id to HTML as they are not needed
...
def result():
    ...
    if(session['given']==0):
        ...
        result = execute(f'''select marks,time_taken from Result where (Result.C_id = '{session['c_id']}') and (Result.S_id = '{session['ID']}') and (Result.E_id = '{session['E_id']}'); ''')
